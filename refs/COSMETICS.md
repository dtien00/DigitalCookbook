...
- **Absent wherever it can't work:** no recognizer (Firefox), no secure context (an `http://` LAN address — how a phone reaches the dev server), or an engine that failed before ever returning words (Brave's disabled backend). The last case toasts once and hides every mic for the page-load rather than leaving buttons that fail on every tap.
