...
**Tag search has no effect on back/forward.** The home root listens for a swipe-left to reopen the last-viewed recipe (Stage 9). A sideways drag already means something else in the search box and the chip group: the strip scrolls, and the search box scrolls a long `tag1, tag2` list or drags a selection. So both regions stop their touches from bubbling to that handler (`swipeExempt` in `HomeView`, the same `stopPropagation` fix RecipeRail uses against RecipeDetail's swipe-back). Before this, flicking through the chips slid the whole page left and navigated away mid-browse. The exemption follows where the touch *starts*: a swipe that begins on the header or grid still reopens the recipe even if the thumb crosses the strip. The browser's own swipe navigation is closed off too. The root's `touch-action: pan-y` normally keeps sideways swipes away from the browser, but a scroll container re-enables sideways panning inside itself. So the collapsed strip is `overscroll-x-contain`, which means a flick past either end stays in the strip and doesn't carry on into Chrome's overscroll back/forward. The expanded wrap never scrolls sideways, so it is simply `touch-pan-y`. Neither affects vertical page scroll.
