...
- **Set sheet** is a bottom sheet (centered on `sm+`) with preset chips (1/3/5/10/15/30 min), matching the CookingMode ingredient-sheet vocabulary. "Add a custom time" reveals a **Dial / Type** toggle (see below). Its Escape handler runs in the **capture phase** with `stopPropagation` so closing it doesn't also trip CookingMode's window-level Escape-to-exit; the panel is `max-h-[92vh] overflow-y-auto` so the taller dial still fits a short phone.

### Clock-dial time picker (Stage 19 Phase 3)

An analog clock face for setting a custom duration alarm-style, offered alongside the plain text field — "Add a custom time" opens a paper-pill **Dial ↔ Type** segmented toggle (Dial default), two editors over one value that carry the duration across when you switch.

- **The face** (`<TimerDial>`, inline SVG on a `#fbf6f1` disc with a `paper-shade` rim) has 60 ticks (majors at the 5s), 1–12 serif numerals in `ink/35` (they double as the 5-unit grid for every hand), a two-tone hub (`ink` + `rust` dot), and up to three hands: hour + minute in `ink`, the second hand in **`rust`** — the palette's stand-in for the spec's "red" second hand, since the rustic palette has no true red (same reasoning as the rose-dark alarm state). Inactive hands sit faint (`ink/35`, `rust/40`); the active hand is full-strength with a `#fbf6f1` knob for a fat touch target.
- **Minutes is the default hand** (≈90% of kitchen timers are "N minutes"); a **Hours / Minutes / Seconds** selector (rust-filled when active) switches which hand drag / the ± steppers move. The second hand only draws once seconds > 0 or Seconds is active, so the common case stays a clean two-hand clock.
- **Setting** is snap-drag via Pointer Events (mouse + touch + pen, like `useDragSort`) — hours snap to 1, minutes to 1, seconds to 5 — plus **± steppers** flanking the `font-serif tabular-nums` readout (the readout is the source of truth) that give a non-drag / keyboard path and echo the ServingsStepper ± discs. Hands are **independent and clamped (no carry)**: dragging minutes past 12 o'clock does not roll into hours, which enforces the ceiling. The active hand rotates with a `transition-transform` that `motion-reduce` disables and a drag suppresses (so it tracks the finger 1:1).
- **Under-12h note.** A persistent italic-rose caption — *"Covers up to 11:59:55 — switch to Type for longer"* — states the dial's range; at the 11-hour ceiling it emphasises to upright `rose-dark` *"Dial maxes out under 12 h…"*. Durations ≥ 12h (overnight ferments, sous-vide) belong to the Type field, not this foreground kitchen timer.
- **Accessibility.** The Type toggle is the keyboard / screen-reader path; the ± steppers are real buttons, the readout is `aria-live="polite"`, and the face carries a descriptive `role="group"` label. No new tokens, no schema — the dial just produces `durationMs` into the existing `onStart`.
...
## Step dictation (per-step mic)

Each step head carries a microphone button (`.step-dictate`) immediately left of the remove `×`, the two inside a `.step-head-actions` wrapper — `.step-head` spaces its children apart (`justify-content: space-between`), so a third bare child would have floated the mic to the middle of the row. Tapping it runs the browser's speech recognizer ([useSpeechDictation.js](../src/hooks/useSpeechDictation.js)) and appends what was heard to that step's instruction ([dictation.js](../src/lib/dictation.js) `appendTranscript`: one space, a capital only where a sentence starts, never an invented period).

- **At rest** the mic matches the `×` beside it — muted `#9a8a7d` glyph, `#f7f1ec` fill, `#e8dcd2` border, 38×30 — so the pair reads as one cluster. **On hover** it warms to rust like the Dial pill (`#fbf6f1` fill, `#b06452` border and glyph), deliberately *not* to the `×`'s `#f2e0da` pink: pink stays delete's alone.
- **Listening** fills it rust (`#b06452`, `#fbf6f1` glyph) and pulses through `motion-safe:animate-pulse` — the TimerWidget alarm's precedent — so a reduced-motion reader gets the fill without the pulse. `aria-pressed` carries the state; the label stays "Dictate step N" (a toggle's name shouldn't flip with its state) while the tooltip switches to "tap to stop and keep what was heard". The resting tooltip is where the app owns the privacy note: the browser's speech service (Google's in Chrome, Apple's in Safari) turns the audio into text.
- **The preview** — `Listening… “what it's heard so far”` — sits under the step head in rose italic (`#a2666f`), the editor's "not real yet" voice (cf. the empty Unit trigger). Only final words touch the textarea, which is why the preview is its own line rather than live text in the field. It's `aria-hidden`; one `sr-only` status line in the Steps section announces "Listening for step N." and "Added to step N." instead of reading out every partial word.
- **The mic skips the remove button's one-step rule.** The `×` hides when a single step remains; the mic never does — dictating the only step is the normal case — so a one-step recipe shows the mic alone at the right edge.
- **Phone (≤640px):** both step-head buttons go to 44×44 with 8px between them (the adjacent-tap-target gap the hub's filter buttons keep). The step `×` had shipped at 38×30 with no phone rule; beside a button that gets tapped constantly, and with no undo for a removed step, it needed the full target. After words land, a phone does *not* focus the textarea — that would raise the keyboard over the form; desktop puts the caret after the new words unless the author has already moved to another field.
- **Absent wherever it can't work:** no recognizer (Firefox), no secure context (an `http://` LAN address — how a phone reaches the dev server), an engine that failed before ever returning words (Brave's disabled backend reports `network`), or one that keeps coming back empty without saying why (Opera exposes the constructor, but its recognizer never returns a result or an error — two empty sessions in a row before any words count as proof, while a stop within 3 s of starting is a change of mind and doesn't count). Those last cases toast once and hide every mic for the page-load rather than leaving buttons that fail on every tap.

Every dictation toast shares one slot (`id: 'dictation'`), so tapping a mic against a blocked microphone replaces the message instead of stacking copies of it.
...
## Allergen / dietary chips (Stage N)

Two chip-multiselect groups sit in the Basic Info section below Tags: **Allergens** ("this recipe contains") over the fixed canonical list from [src/lib/dietaryTags.js](../src/lib/dietaryTags.js), and **Dietary** ("this recipe is") over Vegetarian / Vegan. Each option is a rounded-full toggle `<button>`; a small italic hint under the allergen group states that authors declare these and they are never guessed from ingredients (the safety contract, surfaced at the point of authoring).

**Selected-state color is semantic and driven by inline styles, not utilities.** Allergen ON = danger red (`#b91c1c`, white text) so "contains" reads as a caution rather than a neutral tag; Dietary ON = affirmative green (`#047857`); both OFF = quiet paper outline (`#fff` bg, `#4b5563` text, `#d1d5db` border). The colors are inline `style` objects (`CHIP_ON_ALLERGEN` / `CHIP_ON_DIETARY` / `CHIP_OFF` in [CreateRecipe.jsx](../src/components/CreateRecipe.jsx)) rather than Tailwind classes for a concrete reason worth recording: **this component's `<button>`s are background-zeroed by Tailwind Preflight, and its palette utilities aren't reliably JIT-emitted here** (the same reason the visibility toggle and ingredient-remove use hand-written CSS classes, not utilities). A safety-critical on/off affordance can't hang on a utility class resolving, so the state colors are inlined — guaranteed to paint. This is a functional exception to the "CreateRecipe keeps legacy utilities" deferral, not a retint.

## Dietary filter modal (Stage N)
...
**Active-filter indicator (Stage N item 4).** [DietaryFilterIndicator.jsx](../src/components/DietaryFilterIndicator.jsx) is a persistent banner in the home grid header (below the action row, above the grid *and* the empty state) shown whenever a dietary filter is active — a rose-tinted strip (`border-rose-dark/30 bg-rose-dark/5 rounded-lg`) carrying a "⚠ Excluding" run of rose-dark allergen chips and a "Requiring" run of rust dietary chips, each chip an individual `×` remove, with a right-aligned **Clear all**. This is the deliberate contrast with the Fridge Basket: a convenience filter may narrow silently, but a *safety* filter that hides recipes must announce itself — hence the always-visible named chips (not just the trigger's count badge), the ⚠ glyph, and `role="status"` + `aria-live="polite"` so the exclusions are spoken as they change. Silent filtering is the antipattern the banner exists to prevent.

**Recipe-detail safety banner (Stage N item 5).** [RecipeAllergenWarning.jsx](../src/components/RecipeAllergenWarning.jsx) renders at the top of the `.recipe-detail-header` (above the hero image and the title) when the recipe declares an allergen the viewer has excluded — the direct-link case the grid filter can't cover. Same rose-tinted safety treatment as the indicator (`border-rose-dark/40 bg-rose-dark/10`, ⚠ glyph, rose-dark emphasis on "This recipe contains {list}"), with an `×` dismiss at the right. Distinct from the home indicator in two ways: it's `role="alert"` (assertive — a per-recipe hazard warrants immediate announcement, vs the indicator's polite status), and it's dismissible **per-session, per-recipe** (sessionStorage keyed by recipe id) so acknowledging one recipe doesn't mute the next. `.no-print` — it's a screen-time safety cue, not part of the kitchen card.
