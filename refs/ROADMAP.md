...
- [ ] Mobile audit: cook-in-the-kitchen use case. Make sure the detail page reads well on a phone propped against the kettle. *In progress on `mobile` branch. Vite dev server now binds to LAN by default (`server: { host: true }` in [vite.config.js](./vite.config.js)) so a phone on the same Wi-Fi can hit `http://<lan-ip>:5173`. Landed so far: responsive header title (`text-base sm:text-2xl md:text-3xl` + `truncate min-w-0` so long emails don't blow the layout), a grid density toggle that doubles the column count at every tier (inline copy in the action row + floating copy that fades in once scrolled past the header), a floating scroll-to-top button mirroring the toggle on the right, and infinity-scroll pagination (PAGE_SIZE=20, `count: 'exact'` for stable density tier, IntersectionObserver sentinel below the grid with a 200px lookahead). Still pending: header button overflow (Bookmarks / Profile / Logout cluster wraps awkwardly at narrow widths), tap-target audit on smaller controls, recipe-detail typography review at phone widths.*
